...
for(i in sampleList){
        tmp=fread(paste0(i,"/",i,".hard-filtered-pass-only-TSO500_AGILENT_intersect_notindifficult.vcf.avinput"),sep="\t",data.table=F)
        tmp=tmp[,c(1:5,12,14,15)]
        names(tmp)=c("chr","start","end","ref","alt","FILTER","HEADER","INFO")
        ...
        tmp[,c("DP","VAF")]=t(sapply(1:nrow(tmp),function(x){
			...
			return(c(tmp_info[grep(tmp_header,pattern="DP")],tmp_info[grep(tmp_header,pattern="AF")]))
			
		}))
        tmp$DP=as.numeric(tmp$DP)
        tmp$VAF=as.numeric(tmp$VAF)
        tmp=tmp[,-c(7,8)]
        names(tmp)[-c(1:5)]=paste0(i,c("_FILTER","_DP","_VAF"))
...
result$Pass_Detection_rate=rowSums(!is.na(result[,names(result)[grep(names(result),pattern="^(VAL)[0-9]+_(VAF)$")]]) &
					result[,grep(names(result),pattern="^(VAL)[0-9]+_(FILTER)$")]=="PASS")/8
...
fwrite(result,"HD753-merged-ICA-testing-cases-pass-only-TSO500_AGILENT_intersect_notindifficult.tsv",sep="\t",row.names=F,quote=F)
