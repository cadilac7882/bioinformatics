...
target_regions = read.delim("/Ref/nextera-dna-targeted-regions.bed",sep="\t",header=F)
names(target_regions)=c("chromosome","start","end")
target_regions=target_regions[!target_regions$chromosome%in%c("chrX","chrY"),]

data(exons.hg19)
exons.hg19.GRanges = GenomicRanges::GRanges(seqnames = target_regions$chromosome,
					    IRanges::IRanges(start=target_regions$start,end=target_regions$end))

if(!file.exists("mergedAutosomalBamCount_v2.txt")){
	...
	write.table(myCountsDataFrame,"mergedAutosomalBamCount_v2.txt", sep="\t", col.name=T, row.name=F, quote=F)
}else{
	message("call CNV ...")
	## estimate CNV for specific sample
	myCountsDataFrame = read.delim("mergedAutosomalBamCount_v2.txt",check.names=FALSE)
	...
	for( i in 1:length(all.samples)){
		...
		output.file = paste0(tmpID,'/', tmpID ,'_autosomal_exome_call_v2.csv')
	...
	write.table(corMatrix,"mergedAutosomalCorrelationTable_v2.txt",sep="\t",col.names=F,quote=F)
}
